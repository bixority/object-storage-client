...
async fn test_object_lifecycle() -> anyhow::Result<()> {
    ...
    assert!(list.iter().any(|item| item == "test_file.txt"), "File should be in the list");
    ...
    assert!(!list_after_move.iter().any(|item| item == "test_file.txt"), "Old file should NOT be in the list");
    assert!(list_after_move.iter().any(|item| item == "moved_test_file.txt"), "Moved file should be in the list");
    ...
    assert!(!list_after_delete.iter().any(|item| item == "moved_test_file.txt"), "Deleted file should NOT be in the list");
