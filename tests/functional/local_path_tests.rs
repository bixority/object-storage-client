...
async fn test_absolute_path_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(client.exists(path_str).await?, "File should exist at absolute path");
...
async fn test_relative_path_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(client.exists("Cargo.toml").await?, "Cargo.toml should be visible via relative path");
    
    // We can also check a non-existent relative path
    assert!(!client.exists("non_existent_file_in_cwd.txt").await?, "Non-existent relative path should return false");
...
async fn test_tilde_expansion_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(!client.exists(non_existent).await?, "Non-existent tilde path should return false, not error");
...
async fn test_mixed_local_paths() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(client.exists(&file_url).await?, "Should be visible via file:// URL");
...
async fn test_single_letter_scheme_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(!err_str.contains("UnsupportedScheme(\"c\")"), 
                "Single-letter scheme should be treated as local path, not unsupported scheme: {}", err_str);
        },
        Ok(true) => {
